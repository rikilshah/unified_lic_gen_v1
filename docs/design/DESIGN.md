...
| 2026-07-18 | Dual-card shared serial session | One COM connection probes ASM slave 1 and Stepper slave 2 independently; per-card authorization prevents one trusted manifest from unlocking the other detected card. |
| 2026-07-18 | Compact live control workspace | Denser cards, smaller headings, direct output toggles, animated state cues, live ASM input polling, and task-focused labels improve control-room scanning without removing Admin provisioning. |
| 2026-07-18 | WS2812 visual workflow | A compact color picker is the canonical RGB source for selected/all LED writes; brightness, clear, and cancellable rainbow actions remain grouped with lighting controls. |
| 2026-07-18 | Manifest preference restoration | The last imported manifest is restored per user, with an optional packaged default, while authorization still requires live identity validation. |
| 2026-07-18 | Card-specific public identity groups | The title bar lists detected serials; the compact header keeps ASM and Stepper serial/Customer ID pairs aligned and visually separate so operators cannot log the value against the wrong card. |
| 2026-07-21 | Technician v1.0.2 UI reconciled upstream | The Admin app adopts the compact vertical-axis Stepper workspace, aligned action sizing, live limit cues, footer authorization status, and failure notifications while retaining the complete provisioning and key-generation workflow. |
| 2026-07-21 | Live controls fail closed | Repeated polling or identity failures mark values unknown and revoke only the affected card's authorization; every write revalidates the live identity against the trusted manifest state. |
